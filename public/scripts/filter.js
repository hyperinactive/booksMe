...
function filterReverse(arg, nodes, arg_code) {
    ...
    if ($("#filter-btn-" + arg).children().hasClass("btn-dark")) {
        
        $("#filter-btn-"+arg).children().removeClass("btn-dark");
        $("#filter-btn-" + arg).children().addClass("btn-light");
        ...
        $("#filter-btn-"+arg).children().addClass("btn-dark");
        $("#filter-btn-"+arg).children().removeClass("btn-light");
...
function sortDOMAlpha(nodeList, typeOfField, typeOfSort) {
    ...
    let na = an.sort(function (a, b) {
        var im = a.children[0].children[typeOfField].innerHTML.toString().localeCompare(b.children[0].children[typeOfField].innerHTML.toString());
...
function sortDOMNum(nodeList, typeOfField,typeOfSort) {
    ...
    if (reverseUno) {
        let na = an.sort(function (a, b) {
            if (parseInt(a.children[0].children[typeOfField].innerHTML) > parseInt(b.children[0].children[typeOfField].innerHTML)) {
                return 1;
            } else if (parseInt(a.children[0].children[typeOfField].innerHTML) < parseInt(b.children[0].children[typeOfField].innerHTML)) {
                return -1;
            } else return 0;
        });
